...
from processing_utils import preprocess, generate_visualizations, generate_file_output, generate_cell_state_visualization, validate_variable, generate_weights_visualization
...
def train_lstm_on_site(zip_info):
    ...
    processed = data[variables + train_labels + labels].astype('float32')
    ...
    tuned_parameters = [{'hidden_dim': [50, 100, 256], 'batch_size': [1, 10, 30], 'seq_len': [5, 15, 30], 'clip': [5, 10], 'lr': [0.005, 0.0005, 0.00005, 0.000005]}]
    ...
    generate_weights_visualization(clf.best_estimator_, variables[:-1], zip_info[0])
